...
//
//	$Id: //depot/bc/bluesuite_2_4/devHost/dfu/DFUEngine/CRC.cpp#1 $
//
///////////////////////////////////////////////////////////////////////
...
// Version string
const char *crc_id = "$Id: //depot/bc/bluesuite_2_4/devHost/dfu/DFUEngine/CRC.cpp#1 $ Copyright (C) 2000-2006 Cambridge Silicon Radio Limited";

/*
The following CRC table was copied from the USB Device Firmware Upgrade
Specification, Revision 1.0. The original comment follows:
...
// Constructor
CRC::CRC(uint32 initCRC)
{
	crc = initCRC;
}

// Update the CRC and return the current value
uint32 CRC::operator()(const void *buffer, uint32 bufferLength)
{
	// Include the new data within the CRC
	uint8 *ptr = (uint8 *) buffer;
	for (uint32 index = 0; index < bufferLength; ++index)
	{
		crc = tableCRC[(crc ^ ptr[index]) & 0xff] ^ (crc >> 8);
	}
